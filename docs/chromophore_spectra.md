...
nanometres. Exact table knots return the stored sample; values between knots are
linearly interpolated in continuous wavelength. Queries outside the measured
450–1000 nm interval return `TransportError::WavelengthOutOfRange` rather than
silently clamping or extrapolating.
...
not widen the arithmetic to a different scalar type. This keeps the provider's
CPU and downstream staging contracts explicit.

## Absorption validation

`hemoglobin_absorption` combines tetramer-molar oxy/deoxy concentrations using
Beer–Lambert and converts the centimetre-based spectrum to SI reciprocal metres.
Each concentration is validated before arithmetic, so a negative or non-finite
input cannot be hidden by a positive counterpart. Hyperion returns its typed
...
lookup and typed absorption path together.

...
pub fn coefficient<T: RealField + UnitScalar, Role: CoefficientRole>(
    value_per_meter: f64,
...
pub fn path<T: RealField + UnitScalar>(meters: f64) -> PathLength<T> {
    PathLength::new(Length::from_unit::<Meter>(T::from_f64(meters)))
        .expect("fixture path is finite and non-negative")
}

pub fn photon_energy<T: RealField + UnitScalar>(megaelectronvolts: f64) -> PhotonEnergy<T> {
...
pub fn fluence<T: RealField + UnitScalar>(joules_per_square_meter: f64) -> EnergyFluence<T> {
